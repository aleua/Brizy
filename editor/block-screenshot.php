...
class Brizy_Editor_BlockScreenshot {
	...
	public function save_block_screenshot() {
		...
		if ( empty( $_POST['block_type'] ) || in_array( $_POST['block_type'], self::BLOCK_TYPES ) || empty( $_POST['img'] ) || empty( $_POST['block_id'] ) ) {
			wp_send_json( array( 'success' => false, 'message' => esc_html__( 'There are no all required POST variables', 'brizy' ) ) );
		}
		...
		if ( preg_match( '/^data:image\/(\w+);base64,/', $_POST['img'], $img_type ) ) {
			...
			if ( ! in_array( $img_type, array( 'jpg', 'jpeg', 'gif', 'png' ) ) ) {
				wp_send_json( array( 'success' => false, 'message' => esc_html__( 'Wrong type of block screenshot.', 'brizy' ) ) );
			}

			$base64 = base64_decode( $base64 );

			if ( false === $base64 ) {
				wp_send_json( array( 'success' => false, 'message' => esc_html__( 'Failed to decode block screenshot base64.', 'brizy' ) ) );
			}
		} else {
			wp_send_json( array( 'success' => false, 'message' => esc_html__( 'There is no base64.', 'brizy' ) ) );
		}
		...
		if ( ! file_exists( $path ) ) {
			if ( ! mkdir( $path, 755, true ) ) {
				wp_send_json( array( 'success' => false, 'message' => esc_html__( 'Can not be created the folder blocks. Please check permissions of the folder uploads.', 'brizy' ) ) );
			}
		}
		...
		if ( ! is_writable( $img_path ) || ! file_put_contents( $img_path, $img_base64 ) ) {
			wp_send_json( array( 'success' => false, 'message' => esc_html__( 'Can not be created the block screenshot.', 'brizy' ) ) );
		}
	...
	public function load_block_screenshot() {
		...
		if ( empty( $_GET['block_id'] ) || empty( $_GET['block_type'] ) || in_array( $_GET['block_type'], self::BLOCK_TYPES ) ) {
			wp_send_json( array( 'success' => false, 'message' => esc_html__( 'There are no all required GET variables', 'brizy' ) ) );
		}

		$path = $this->get_dir( $_GET ) . sanitize_file_name( "{$_GET['block_id']}.jpg" );

		if ( ! file_exists( $path ) ) {
			wp_send_json( array( 'success' => false, 'message' => esc_html__( 'The screenshot of the block does not exist.', 'brizy' ) ) );
		}
	...
	private function get_dir( $arr ) {
		...
		if ( in_array( $block_type, self::BLOCK_TYPES ) ) {
			...
			if ( empty( $arr['post_id'] ) || ! is_numeric( $arr['post_id'] ) ) {
				wp_send_json( array( 'success' => false, 'message' => esc_html__( 'Wrong ID of the post.', 'brizy' ) ) );
			}

			$dir = $arr['post_id'];
		}

		return Brizy_Editor_UploadsDir::get_path() . implode( DIRECTORY_SEPARATOR, array( $dir, 'assets', 'thumbnails' ) ) . DIRECTORY_SEPARATOR;
	}
}